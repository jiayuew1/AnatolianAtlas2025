      ...
      const markers = this.points.map(point => {
        const marker = L.circleMarker(
          ...
          <div class="popup-content">
            <h3>${point['Name']}</h3>
            <p><strong>Anatolian Atlas ID:</strong> ${point['AA_ID'] || point['OBJECTID'] || ''}</p>
            <p><strong>Survey:</strong> ${point.Survey}</p>
            <p><strong>Periods:</strong> ${point.News ? point.News.split(',').join(', ') : point.attestations4Filter ? point.attestations4Filter.split(',').join(', ') : ''}</p>
